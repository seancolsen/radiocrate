...
import { clientBuildId } from "../stores/update";
...
function Row(props: { label: string; value: string }): JSX.Element {
  return (
    <>
      <dt className="text-ink-weak text-sm">{props.label}</dt>
      <dd className="text-ink font-mono text-xs break-all">{props.value}</dd>
    </>
  );
}
...
function VersionVerdict(props: {
  ...
  if (!props.version) {
    return (
      <p className="text-ink-weak mt-3 text-sm">
        {props.checking ? "Checking…" : "Couldn’t reach the server."}
      </p>
    );
  }
  if (props.version.buildId !== DEV_BUILD_ID) {
    if (isClientStale(props.clientBuildId, props.version.buildId)) {
      return (
        <p className="text-danger mt-3 text-sm font-medium">
          This client didn’t come from the server that’s running. Reload to get
          the current one.
        </p>
      );
    }
    return (
      <p className="text-ink-weak mt-3 text-sm">This client is up to date.</p>
    );
  }
  return (
    <p className="text-ink-weak mt-3 text-sm">
      Development server — it serves no embedded client, so there is nothing to
      compare against.
    </p>
  );
}
      ...
      {confirming ? (
        <>
          ...
          <div className="mt-3 flex flex-wrap justify-end gap-2">
            ...
            <button
              type="button"
              className="bg-accent text-panel rounded-md px-3 py-1.5 text-sm"
        ...
        <div className="mt-4 flex flex-wrap justify-end gap-2">
          ...
          <button
            ...
            className="text-ink hover:bg-hover rounded-md px-3 py-1.5 text-sm disabled:opacity-40"
...
function AboutModalBody(): JSX.Element {
  ...
  // Starts `true`: the dialog runs its check immediately on mount (see the
  // effect below), so there's never a moment where `checking` is `false` and
  // the mount check hasn't happened yet. Kept `true` (never flipped back
  // *within* the mount effect's own synchronous body — only in the `.then`
  // below, and in `check`'s event-handler use) so nothing here calls
  ...
  useEffect(() => {
    ...
    void updateActions.checkForUpdate().finally(() => setChecking(false));
  ...
  return (
    <AboutDialog
      ...
      checking={checking}
      onCheck={() => void check()}
      onReloadFresh={() => void updateActions.resetAppData()}
      onClose={() => appActions.closeAbout()}
    />
  );
}
...
export default function AboutModal(): JSX.Element | null {
  ...
  return aboutOpen ? <AboutModalBody /> : null;
}
